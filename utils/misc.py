...
def load_content(data_name: str = "diting_light"):
    try:
        with open('./data/prompt_bank/{0}.txt'.format(data_name), 'r') as f:
            content = f.read()
        return content
    except FileNotFoundError:
        print(f"Warning: Prompt file for dataset '{data_name}' not found.")
        return None
